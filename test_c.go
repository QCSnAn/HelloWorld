...
func (e *RowsEvent) DecodeHeader(data []byte) (int, error) {
	...
	if !ok {
		if len(e.tables) > 0 {
			return 0, fmt.Errorf("invalid table id %d, no corresponding table map event", e.TableID)
		} else {
			return 0, fmt.Errorf("invalid table id, no corresponding table map event, table id %d", e.TableID)
		}
	}
	return pos, nil
}
...
func (d *jsonBinaryDecoder) isDataShort(data []byte, expected int) bool {
	...
	if len(data) < expected {
		d.err = fmt.Errorf("data len %d < expected %d", len(data), expected)
	}
...
func (d *jsonBinaryDecoder) decodeObjectOrArray(data []byte, isSmall bool, isObject bool) interface{} {
	...
	if headerSize > size {
		d.err = fmt.Errorf("header size %d > size %d", headerSize, size)
		return nil
	}
	...
	if isObject {
		...
		for i := 0; i < count; i++ {
			...
			if keyOffset < headerSize {
				d.err = fmt.Errorf("invalid key offset %d, must > %d", keyOffset, headerSize)
				return nil
			}
...
func (d *jsonBinaryDecoder) decodeLiteral(data []byte) interface{} {
	...
	d.err = fmt.Errorf("invalid literal %c", tp)
	...
	for ; pos < maxCount; pos++ {
		...
		if v&0x80 == 0 {
			if length > math.MaxUint32 {
				d.err = fmt.Errorf("variable length %d must <= %d", length, int64(math.MaxUint32))
				return 0, 0
			}
...
func (d *jsonBinaryDecoder) decodeValue(tp byte, data []byte) interface{} {
	...
	default:
		d.err = fmt.Errorf("invalid json type %d", tp)
	}
...
func (e *RowsEvent) decodeJsonPartialBinary(data []byte) (*JsonDiff, error) {
	...
	default:
		return nil, fmt.Errorf("corrupted JSON diff")
	}
	data = data[1:]
...
func (e *RowsEvent) DecodeData(pos int, data []byte) (err2 error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err2 = fmt.Errorf("parse rows event panic %v, data %q, parsed rows %#v, table map %#v", r, data, e, e.Table)
		}
	}()
...
func (e *TableMapEvent) decodeDefaultCharset(v []byte) (ret []uint64, err error) {
	...
	if len(ret)%2 != 1 {
		return nil, fmt.Errorf("Expect odd item in DefaultCharset but got %d", len(ret))
	}
	return
}
...
func (e *TableMapEvent) decodeColumnNames(v []byte) error {
	...
	if len(e.ColumnName) != int(e.ColumnCount) {
		return fmt.Errorf("Expect %d column names but got %d", e.ColumnCount, len(e.ColumnName))
	}
	return nil
}
...
func ParseUUIDSet(str string) (*UUIDSet, error) {
	...
	if len(sep) < 2 {
		return nil, errors.New("invalid GTID format, must UUID:interval[:interval]")
	}
...
func (c *Conn) ReadPacketTo(w io.Writer) error {
	...
	if _, err := c.copyN(b, 4); err != nil {
		return fmt.Errorf("connection was bad io.ReadFull(header) failed. err %v", err)
	} else {
	...
	if n, err := c.copyN(w, int64(length)); err != nil {
		return fmt.Errorf("connection was bad io.CopyN failed. err %v, copied %v, expected %v", err, n, length)
	} else if n != int64(length) {
		return fmt.Errorf("connection was bad io.CopyN failed(n != int64(length)). %v bytes copied, while %v expected", n, length)
	} else {
		...
		if err = c.ReadPacketTo(w); err != nil {
			return fmt.Errorf("ReadPacketTo failed %v", err)
		}
	}
...
func (c *Conn) readInitialHandshake() error {
	...
	if data[0] != ClassicProtocolVersion {
		...
		return fmt.Errorf("invalid protocol version %d, expected 10", data[0])
	}
	pos := 1
	...
	if data[pos] != 0 { // 	0x00 byte, terminating the first part of a scramble
		return fmt.Errorf("expect 0x00 after scramble, got %q", rune(data[pos]))
	}
	pos++
	...
	if len(data) > pos {
		...
		if (c.capability&CLIENT_PLUGIN_AUTH == 0) && (authPluginDataLen > 0) {
			return fmt.Errorf("invalid auth plugin data filler %d", authPluginDataLen)
		}
		pos++
		...
		if c.capability&CLIENT_PLUGIN_AUTH != 0 {
			...
			if data[pos] != 0 {
				return fmt.Errorf("expect 0x00 after authPluginName, got %q", rune(data[pos]))
			}
			// pos++ // ineffectual
		}
	}
...
func (c *Conn) WritePacket(data []byte) error {
	...
	for length >= MaxPayloadLen {
		...
		if n, err := c.Write(data[:4+MaxPayloadLen]); err != nil {
			return fmt.Errorf("connection was bad. Write(payload portion) failed. err %v", err)
		} else if n != (4 + MaxPayloadLen) {
			return fmt.Errorf("connection was bad. Write(payload portion) failed. only %v bytes written, while %v expected", n, 4+MaxPayloadLen)
		} else {
			c.Sequence++
	...
	if n, err := c.Write(data); err != nil {
		return fmt.Errorf("connection was bad. Write failed. err %v", err)
	} else if n != len(data) {
		return fmt.Errorf("connection was bad. Write failed. only %v bytes written, while %v expected", n, len(data))
	}
...
func (c *Conn) WriteAuthSwitchPacket(authData []byte, addNUL bool) error {
	...
	return fmt.Errorf("WritePacket failed, %v", c.WritePacket(data))
}
...
func (c *Conn) WritePublicKeyAuthPacket(password string, cipher []byte) error {
	...
	if err := c.WritePacket(data); err != nil {
		return fmt.Errorf("WritePacket(single byte) failed, %v", err)
	}

	data, err := c.ReadPacketReuseMem(nil)
	if err != nil {
		return fmt.Errorf("ReadPacket failed, %v", err)
	}

	block, _ := pem.Decode(data[1:])
	pub, err := x509.ParsePKIXPublicKey(block.Bytes)
	if err != nil {
		return fmt.Errorf("x509.ParsePKIXPublicKey failed, %v", err)
	}
...
func (c *Conn) WriteEncryptedPassword(password string, seed []byte, pub *rsa.PublicKey) error {
	...
	return fmt.Errorf("WriteAuthSwitchPacket faile, %v", c.WriteAuthSwitchPacket(enc, false))
}
...
func (c *Conn) handleAuthResult() error {
	...
	if switchToPlugin != "" {
		...
		if switchToPlugin != "" {
			return errors.New("can not switch auth plugin more than once")
		}
	}
	...
	if c.authPluginName == AUTH_CACHING_SHA2_PASSWORD {
		...
		if data[0] == CACHE_SHA2_FAST_AUTH {
			...
			return fmt.Errorf("invalid packet %x", data[0])
		}
...
func (c *Conn) handshake() error {
	...
	if err = c.readInitialHandshake(); err != nil {
		c.Close()
		return fmt.Errorf("readInitialHandshake: %w", err)
	}

	if err = c.writeAuthHandshake(); err != nil {
		c.Close()
		return fmt.Errorf("writeAuthHandshake: %w", err)
	}

	if err = c.handleAuthResult(); err != nil {
		c.Close()
		return fmt.Errorf("handleAuthResult: %w", err)
	}
...
func ConnectWithDialer(addr, user, password, dbName string, options ...Option) (*Conn, error) {
	c := new(Conn)
...
func (b *BinlogSyncer) newConnection() (*Conn, error) {
	var addr string
	addr = net.JoinHostPort(b.cfg.Host, strconv.Itoa(int(b.cfg.Port)))

	return ConnectWithDialer(addr, b.cfg.User, b.cfg.Password, "test", func(c *Conn) error {
...
func (f *Field) Parse(p []byte) (err error) {
	...
	if len(p) > pos {
		...
		if pos+int(f.DefaultValueLength) > len(p) {
			err = errors.New("Malform packet error")
			return err
		}
...
func (p RowData) ParseBinary(f []*Field, dst []FieldValue) ([]FieldValue, error) {
	...
	if p[0] != OK_HEADER {
		return nil, errors.New("Malform packet error")
	}
...
func (b *BinlogSyncer) prepare() error {
	if b.isClosed() {
		return errors.New("sync was closed")
	}
...
func (p *BinlogParser) verifyCrc32Checksum(rawData []byte) error {
	...
	if !bytes.Equal(expectedChecksum, computed) {
		return errors.New("binlog checksum mismatch, data may be corrupted")
	}
	return nil
}
...
func (b *BinlogSyncer) StartSyncGTID(gset GTIDSet) (*BinlogStreamer, error) {
	...
	if b.running {
		return nil, errors.New("last sync error or closed, try sync and get event again")
	}
